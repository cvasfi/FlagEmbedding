...
class BGEM3Model(nn.Module):
    ...
    def load_model(self, model_name, colbert_dim: int = -1):
        ...
        ) and os.path.exists(os.path.join(model_name, "sparse_linear.pt")):
        ...
        if self.training:
            if teacher_scores is not None:
                ...
                if self.negatives_cross_device:
                    ...
                    cross_dense_scores = self.dense_score(
                        cross_q_dense_vecs, cross_p_dense_vecs
                    )
                    ...
                    loss = self.distill_loss(
                        teacher_targets, dense_scores, group_size=group_size
                    )

                if self.unified_finetuning:
                    # sparse and colbert loss
                    sparse_scores = self.sparse_score(
            ...
            if (
                ...
                ensemble_distill_dense_loss = -torch.mean(
                    torch.sum(
                        torch.log_softmax(dense_scores, dim=-1) * teacher_targets,
                        dim=-1,
                    )
                )
                ensemble_distill_sparse_loss = -torch.mean(
                    torch.sum(
                        torch.log_softmax(sparse_scores, dim=-1) * teacher_targets,
                        dim=-1,
                    )
                )
                ensemble_distill_colbert_loss = -torch.mean(
                    torch.sum(
                        torch.log_softmax(colbert_scores, dim=-1) * teacher_targets,
                        dim=-1,
                    )
                )
                loss += (
    ...
    def forward(
        ...
        if self.normlized:
            if "dense_vecs" in output:
                output["dense_vecs"] = torch.nn.functional.normalize(
                    output["dense_vecs"], dim=-1
                )
