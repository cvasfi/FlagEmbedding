...
class ModelArguments:
    ...
    resume_peft: bool = field(
        default=False, metadata={"help": "Load model from a PEFT checkpoint?"}
    )
...
class DataArguments:
    ...
    train_data: str = field(
        default=None,
        metadata={"help": "One or more paths to training data", "nargs": "+"},
    )
    ...
    same_task_within_batch: bool = field(
        default=False,
        metadata={"help": "All samples in the same batch comes from the same task."},
    )
        ...
        for train_dir in self.train_data:
            if not os.path.exists(train_dir):
                raise FileNotFoundError(
                    f"cannot find file: {train_dir}, please set a true path"
                )
