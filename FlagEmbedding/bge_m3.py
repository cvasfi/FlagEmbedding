...
class BGEM3FlagModel:
    ...
    def encode_to_disk(
        ...
        pandas_store_prefix="embeddings",
    ) -> Dict:
        ...
        with pd.HDFStore(
            f"{pandas_store_prefix}_dense.h5", mode="w"
        ) as dense_store, pd.HDFStore(
            f"{pandas_store_prefix}_sparse.h5", mode="w"
        ) as sparse_store, pd.HDFStore(
            f"{pandas_store_prefix}_colbert.h5", mode="w"
        ) as colbert_store:
            for start_index in tqdm(
                ...
                def add_to_store(store: pd.HDFStore, elements, columns=None):
                    for idx, el in enumerate(elements):
                        store[f"e_{start_index + idx}"] = pd.DataFrame(
                            el, columns=columns
                        )

                if return_dense:
                    add_to_store(dense_store, output["dense_vecs"].cpu().numpy())
                ...
                if return_sparse:
                    ...
                    # Storing them as tuples to make them convertable to dataframe
                    sparse_embeddings = [list(e.items()) for e in sparse_embeddings]
                    add_to_store(
                        sparse_store,
                        sparse_embeddings,
                        ["key", "value"],
            ...
            return {
                "dense_vecs": dense_store.filename,
                "lexical_weights": sparse_store.filename,
                "colbert_vecs": colbert_store.filename,
            }
    ...
    def compute_score(
        ...
        for start_index in tqdm(
            ...
            queries_inputs = _tokenize(queries_batch, max_length=max_query_length).to(
                self.device
            )
            corpus_inputs = _tokenize(corpus_batch, max_length=max_passage_length).to(
                self.device
            )
            ...
            colbert_scores = self.model.colbert_score(
