...
class DatasetForReranker(Dataset):
    def __init__(
        ...
        self.encode_max_length = (
            self.max_len + len(self.sep_inputs) + len(self.prompt_inputs)
        )
...
class collater:
    ...
    def __call__(self, data):
        labels = (
            [feature["labels"] for feature in data]
...
class FlagLLMReranker:
    ...
    def compute_score(
        ...
        length_sorted_idx = np.argsort(
            [-self._text_length(q) - self._text_length(p) for q, p in sentence_pairs]
        )
        ...
        if dataloader is not None:
            ...
            for batch_start in trange(0, len(sentences_sorted), batch_size):
                batch_sentences = sentences_sorted[
                ...
                for query_inputs, passage_inputs in zip(
                    queries_inputs["input_ids"], passages_inputs["input_ids"]
                ):
                    item = self.tokenizer.prepare_for_model(
                ...
                batch_inputs = {
                    key: val.to(self.device) for key, val in batch_inputs.items()
                }
...
class LayerWiseFlagLLMReranker:
    def __init__(
        ...
        if device and isinstance(device, str):
            if device == "cpu":
                warnings.warn(
                    "The LLM-based layer-wise reranker does not support CPU; it has been set to CUDA."
                )
                device = "cuda"
            self.device = torch.device(device)
        else:
    ...
    def compute_score(
        ...
        if dataloader is not None:
            ...
            for batch_start in trange(0, len(sentences_sorted), batch_size):
                ...
                tmp_all_scores = []
                for logits in all_logits:
                    scores = last_logit_pool_layerwise(
        ...
        for i in range(len(all_scores)):
            all_scores[i] = [
                all_scores[i][idx] for idx in np.argsort(length_sorted_idx)
            ]
            if normalize:
        ...
        for batch_start in trange(0, len(sentences_sorted), batch_size):
            ...
            for query_inputs, passage_inputs in zip(
                ...
                query_lengths.append(
                    len([self.tokenizer.bos_token_id] + query_inputs + sep_inputs)
                )
            ...
            for i in range(len(outputs.logits)):
                logits = last_logit_pool_lightweight(
                    outputs.logits[i], outputs.attention_masks[i]
                )
